from transformers import AutoMOdelForSpa, BertForSequenceClassification, AutoTokenizer
...
def create_model(name: str, label_column: str, dataset: Dataset) -> RunParameters:
    ...
    if name.startswith("hubert-wiki"):
        ...
        if name.endswith("uncased"):
            model_type = 'uncased'
        model = load_hubert(model_type, BertForSequenceClassification, config)
        tokenizer = load_tokenizer(model_type)
    else:
        model = AutoModelForSequenceClassification.from_pretrained(name, **config)
